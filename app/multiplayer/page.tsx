...
export default function MultiplayerPage() {
  ...
  const copyLobbyCode = async () => {
    ...
    setError('');
    try {
      if (navigator.clipboard?.writeText) await navigator.clipboard.writeText(lobby.code);
      else throw new Error('Clipboard API unavailable');
    } catch {
      const helper = document.createElement('textarea');
      helper.value = lobby.code; helper.style.position = 'fixed'; helper.style.opacity = '0';
      document.body.appendChild(helper); helper.select();
      const copiedWithFallback = document.execCommand('copy'); helper.remove();
      if (!copiedWithFallback) { setError(`No se pudo copiar. Código: ${lobby.code}`); return; }
    }
  ...
  const returnToLobby = async () => {
    if (!lobby) return;
    setWorking(true); setError('');
    const { error: resetError } = await supabase.rpc('reset_lobby', { target_lobby: lobby.id });
    if (resetError) { setError(resetError.message); setWorking(false); return; }
    setLocalFinished(false); setStartedAt(0); setCountdown(null);
    setTyped(''); setScore(0); setCombo(0); setMaxCombo(0); setCorrect(0); setMistakes(0);
    submittedRef.current = false; lastProgressRef.current = ''; lastLineRef.current = 0; lastTypedLength.current = 0;
    await loadRoom(lobby.id); setWorking(false);
  };
  ...
  useEffect(() => {
    ...
    const publish = async () => {
      ...
      if (signature === lastProgressRef.current || publishingProgressRef.current) return;
      publishingProgressRef.current = true;
      setPlayers(current => current.map(player => player.user_id === authUser?.id ? {
        ...player, score: stats.score, accuracy: Math.round(stats.accuracy * 10) / 10,
        wpm: stats.wpm, max_combo: Math.max(player.max_combo, stats.combo),
      } : player));
      const { error: progressError } = await supabase.rpc('update_lobby_progress', {
      ...
      if (progressError) { setError(`No se pudo sincronizar la puntuación: ${progressError.message}`); return; }

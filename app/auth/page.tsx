  ...
  useEffect(()=>{
    const finishConfirmation=async()=>{
      ...
      if(code){
        ...
        if(exchangeError){setError('El enlace de confirmación venció o ya fue utilizado. Solicitá uno nuevo.');setCheckingLink(false);return;}
  ...
  const submit=async(event:FormEvent)=>{
    ...
    try {
      if(signup){
        ...
        if(!/^[a-zA-Z0-9_.-]+$/.test(cleanUsername)) throw new Error('Usá solamente letras, números, puntos, guiones y guion bajo.');
      ...
      setError(
        message.toLowerCase().includes('email rate limit')
          ? 'Supabase alcanzó el límite temporal de correos. Si ya recibiste una confirmación, usá ese enlace; de lo contrario esperá hasta una hora antes de solicitar otra.'
          : message.includes('Invalid login')
            ? 'Correo o contraseña incorrectos.'
            : message.includes('already registered')
              ? 'Este correo ya está registrado. Revisá el correo de confirmación o iniciá sesión.'
              : message
      );
